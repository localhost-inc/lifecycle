...
export class LocalRuntime implements WorkspaceRuntime {
  private invoke: LocalRuntimeDeps["invoke"];
  private watchPath: LocalRuntimeDeps["watchPath"];

  constructor(deps: LocalRuntimeDeps) {
  ...
  async sendTerminalText(workspaceId: string, terminalId: string, text: string): Promise<void> {
    await this.invoke("send_terminal_text", {
      workspaceId,
      terminalId,
      text,
    });
