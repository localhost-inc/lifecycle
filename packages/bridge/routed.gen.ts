// ⚠️ Auto-generated by routed. Do not edit.
import { defineRouteTree } from "routedjs";
import { Hono } from "hono";
import { routeHandler, wrapMiddleware } from "routedjs/hono";

import middleware0 from "./routes/_middleware.ts";

import route0 from "./routes/health.get.route.ts";
import route1 from "./routes/organizations.get.route.ts";
import route2 from "./routes/organizations.post.route.ts";
import route3 from "./routes/organizations/$orgId/cloud-accounts.post.route.ts";
import route4 from "./routes/repos.get.route.ts";
import route5 from "./routes/repos.post.route.ts";
import route6 from "./routes/repos/$repoId.delete.route.ts";
import route7 from "./routes/repos/link.post.route.ts";
import route8 from "./routes/users/me/environment.put.route.ts";
import route9 from "./routes/workspaces.get.route.ts";
import route10 from "./routes/workspaces/index.post.route.ts";
import route11 from "./routes/workspaces/$id.delete.route.ts";
import route12 from "./routes/workspaces/$id/exec.post.route.ts";
import route13 from "./routes/workspaces/$id/pr.post.route.ts";
import route14 from "./routes/workspaces/$id/pr/merge.post.route.ts";
import route15 from "./routes/workspaces/$id/services.get.route.ts";
import route16 from "./routes/workspaces/$id/shell.get.route.ts";
import route17 from "./routes/workspaces/$id/shell.post.route.ts";
import route18 from "./routes/workspaces/activity.get.route.ts";

export const routeTree = defineRouteTree([
  {
    path: "/health",
    method: "get",
    route: route0,
    middleware: [middleware0],
  },
  {
    path: "/organizations",
    method: "get",
    route: route1,
    middleware: [middleware0],
  },
  {
    path: "/organizations",
    method: "post",
    route: route2,
    middleware: [middleware0],
  },
  {
    path: "/organizations/:orgId/cloud-accounts",
    method: "post",
    route: route3,
    middleware: [middleware0],
  },
  {
    path: "/repos",
    method: "get",
    route: route4,
    middleware: [middleware0],
  },
  {
    path: "/repos",
    method: "post",
    route: route5,
    middleware: [middleware0],
  },
  {
    path: "/repos/:repoId",
    method: "delete",
    route: route6,
    middleware: [middleware0],
  },
  {
    path: "/repos/link",
    method: "post",
    route: route7,
    middleware: [middleware0],
  },
  {
    path: "/users/me/environment",
    method: "put",
    route: route8,
    middleware: [middleware0],
  },
  {
    path: "/workspaces",
    method: "get",
    route: route9,
    middleware: [middleware0],
  },
  {
    path: "/workspaces",
    method: "post",
    route: route10,
    middleware: [middleware0],
  },
  {
    path: "/workspaces/:id",
    method: "delete",
    route: route11,
    middleware: [middleware0],
  },
  {
    path: "/workspaces/:id/exec",
    method: "post",
    route: route12,
    middleware: [middleware0],
  },
  {
    path: "/workspaces/:id/pr",
    method: "post",
    route: route13,
    middleware: [middleware0],
  },
  {
    path: "/workspaces/:id/pr/merge",
    method: "post",
    route: route14,
    middleware: [middleware0],
  },
  {
    path: "/workspaces/:id/services",
    method: "get",
    route: route15,
    middleware: [middleware0],
  },
  {
    path: "/workspaces/:id/shell",
    method: "get",
    route: route16,
    middleware: [middleware0],
  },
  {
    path: "/workspaces/:id/shell",
    method: "post",
    route: route17,
    middleware: [middleware0],
  },
  {
    path: "/workspaces/activity",
    method: "get",
    route: route18,
    middleware: [middleware0],
  },
]);

export const app = new Hono()
  .use("*", wrapMiddleware(middleware0))
  .get("/health", routeHandler(route0, "/health"))
  .get("/organizations", routeHandler(route1, "/organizations"))
  .post("/organizations", routeHandler(route2, "/organizations"))
  .post("/organizations/:orgId/cloud-accounts", routeHandler(route3, "/organizations/:orgId/cloud-accounts"))
  .get("/repos", routeHandler(route4, "/repos"))
  .post("/repos", routeHandler(route5, "/repos"))
  .delete("/repos/:repoId", routeHandler(route6, "/repos/:repoId"))
  .post("/repos/link", routeHandler(route7, "/repos/link"))
  .put("/users/me/environment", routeHandler(route8, "/users/me/environment"))
  .get("/workspaces", routeHandler(route9, "/workspaces"))
  .post("/workspaces", routeHandler(route10, "/workspaces"))
  .delete("/workspaces/:id", routeHandler(route11, "/workspaces/:id"))
  .post("/workspaces/:id/exec", routeHandler(route12, "/workspaces/:id/exec"))
  .post("/workspaces/:id/pr", routeHandler(route13, "/workspaces/:id/pr"))
  .post("/workspaces/:id/pr/merge", routeHandler(route14, "/workspaces/:id/pr/merge"))
  .get("/workspaces/:id/services", routeHandler(route15, "/workspaces/:id/services"))
  .get("/workspaces/:id/shell", routeHandler(route16, "/workspaces/:id/shell"))
  .post("/workspaces/:id/shell", routeHandler(route17, "/workspaces/:id/shell"))
  .get("/workspaces/activity", routeHandler(route18, "/workspaces/activity"))
;

export type AppType = typeof app;
