...
import { stackLogFilePath, upsertStackRuntimeService } from "../stack";
import { createWorkspaceHostRegistry, type WorkspaceHostAdapter } from "../workspace";

import { readWorkspaceLogs } from "./logs";
...
describe("readWorkspaceLogs", () => {
  test("reads service logs from the canonical repo/workspace slug path", async () => {
    ...
    const result = await readWorkspaceLogs(db, workspaceRegistry, workspace.id, { tail: 20 });
