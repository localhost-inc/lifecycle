...
const runner: AgentWorkerCommandRunner = {
...
function createTestAgentWorker() {
  return createAgentWorker({
    commandRunner: runner,
    async attachSessionConnection() {
      throw new Error("attachSessionConnection is not used in worker tests.");
    },
    async startSessionConnection() {
      throw new Error("startSessionConnection is not used in worker tests.");
    },
  });
}
...
describe("agent worker", () => {
  ...
  test("checks provider auth through lifecycle cli", async () => {
    ...
    const status = await client.checkAuth("codex");
  ...
  test("streams auth status updates while logging in", async () => {
    ...
    const loginPromise = client.login("codex", (status) => {
      updates.push(status);
    });
  ...
  test("fetches the provider model catalog through lifecycle cli", async () => {
    ...
    const catalog = await client.getModelCatalog("claude", {
      loginMethod: "console",
    });
  ...
  test("does not pass stale preferred-model flags for codex catalog reads", async () => {
    ...
    const catalog = await client.getModelCatalog("codex", {
  ...
  test("surfaces catalog command failures", async () => {
    ...
    await expect(client.getModelCatalog("codex", {})).rejects.toThrow("catalog failed");
  });
});
