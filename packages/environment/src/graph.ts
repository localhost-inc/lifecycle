...
export type EnvironmentNodeKind = "task" | "service";
...
function collectTransitiveDeps(
  ...
  for (const dep of nodeDependsOn(node)) {
    collectTransitiveDeps(environment, dep, selected, satisfied);
  }
}
...
export function lowerEnvironmentGraph(
  ...
  if (options.targetServices && options.targetServices.length > 0) {
    ...
    for (const name of options.targetServices) {
      const node = config.environment[name];
      if (!node) {
  ...
  for (const [name, node] of Object.entries(config.environment)) {
  ...
  for (const [name, nodeConfig] of Object.entries(config.environment)) {
...
export function resolveStartOrder(
  config: LifecycleConfig,
  options: LowerOptions,
): { prepareSteps: LoweredGraph["prepareSteps"]; sorted: EnvironmentNode[] } {
  const graph = lowerEnvironmentGraph(config, options);
