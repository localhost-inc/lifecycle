...
11. External local terminal session hosting so app restart can reattach live terminal sessions by `terminal.id` instead of recreating them inside Tauri.
...
1. Local terminal creation now provisions an external `tmux` session before the `terminal` row is returned.
2. Native surface sync is attach-only and reconnects Ghostty to the existing `tmux` session keyed by `terminal.id`.
3. Desktop app restart now preserves live local shell and harness sessions as long as the external session host remains available.
