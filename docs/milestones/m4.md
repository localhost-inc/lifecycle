...
> Prerequisites: M3
> Introduces: run/reset semantics, local sleep/wake, local destroy, local preview, mutation locking, terminal sleep/wake

## Goal

The complete local workspace lifecycle loop works end-to-end: create -> run -> terminal -> reset -> sleep -> wake -> preview -> destroy -- with mutation locking and terminal state management. No cloud, no auth, no network.
...
6. Terminal sleep/wake transitions.
...
- Terminal tabs show "suspended" during sleep, resume on wake
