...
> Prerequisites: M3
> Introduces: local environment run/reset/sleep/wake/destroy, local preview, mutation locking, terminal/service separation
> Tracker: high-level status/checklist lives in [`docs/plan.md`](../plan.md). This document is the detailed implementation contract.

## Goal

M4 turns a local workspace into a controllable execution environment. Services can be started, reset, slept, woken, previewed, and destroyed cleanly, with mutation locking and clear separation between agent terminals and service runtime state. No cloud, no auth, no network.
...
1. `run` (start or restart the workspace environment's services) and `reset` (restore post-setup baseline).
...
8. Desktop app: local environment controls, destruction confirmation, sleep/wake indicators.
...
- Environment `sleeping` means services are stopped while the worktree remains available for interactive terminal work.
...
- Full local environment loop works: create -> run -> terminal -> reset -> sleep -> wake -> preview -> destroy
