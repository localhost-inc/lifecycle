...
> Update (2026-03-15): M4 moved local terminal session authority into an external `tmux` host keyed by `terminal.id`. The native Ghostty view remains the presentation layer, but desktop app restart no longer terminates live local terminal sessions.
...
3. Historical M3 limitation: local terminals originally did not survive desktop app restart because the runtime lived inside Tauri. M4 lifted that limitation with an external `tmux` session host.
...
3. macOS native mode persists terminal metadata in SQLite while an external `tmux` session host owns the child process lifetime.
4. The native Ghostty surface is attach-only presentation and may reconnect to the same live local session after app restart.
5. Explicit terminal kill tears down the external session host; app shutdown does not.
...
5. App shutdown does not terminate local terminal sessions once the external session host is active; restart reattachment depends on the persisted `terminal.id`.
...
7. Desktop app quit/window close preserves local terminal sessions because the external session host lives outside the Tauri process.
8. The current persistence host is `tmux`; a future daemon may replace it without changing terminal identity or attach semantics.
