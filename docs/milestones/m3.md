...
6. Runtime-backed terminal tabs with live state powered by the desktop query layer.
...
- raw terminal output does **not** flow through the event foundation on the browser fallback path
...
3. PTY output is streamed over Tauri `Channel`, not over the event foundation, on the browser fallback path.
