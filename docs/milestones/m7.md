# Milestone 7: "I can sign in, fork to cloud, share a preview, and create a PR"
...
4. Convex schemas: `organization`, `repository`, `activity`. Selective sync and mirror logic for portable `project` and workspace metadata across local and cloud boundaries, without transferring authority for local runtime state.
...
16. Shared terminal sessions: Durable Object multiplexer, invite flow, role-based input control, presence.
...
`workspace.mode` remains the authority boundary in M7.

1. `local` workspaces continue to use the local provider, local persistence, and local runtime authority even when the user is signed in.
...
6. Sync from local to Convex is for linkage, handoff, and selected mirrored metadata. It is not a mandate to replicate all local runtime state into Convex.
...
- **Text-over-wire**: terminal data is transmitted as PTY text, not video — sub-50ms latency through the Durable Object
...
3. **Terminal attach**: `terminals.mintAttachToken` looks up the user's invite role and embeds it in the token. Host (workspace creator) is always `editor`.
...
1. **Cloudflare routing model**:
   ...
   - route target resolves through cloud runtime metadata to `{sandbox_id, effective_port}` and is reconciled on service restart/wake
...
Commands added to the CLI in this milestone (extending M6's local CLI):
