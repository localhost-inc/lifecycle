# Milestone 6: "I can inspect and control local workspaces from the CLI"
...
A developer (or agent) working inside a workspace worktree can use the Lifecycle CLI to inspect status, tail logs, run health checks, create/run/reset/destroy workspaces, and get a structured context dump -- without passing IDs. No cloud, no auth, no network.
...
3. **Observability commands** (the agent surface):
   - `lifecycle workspace status` -- workspace state, git ref/sha, mode, all services with ports and health, uptime. Default output is a single-screen dashboard. `--json` emits full structured state.
...
#### Workspace Lifecycle (Local)

1. `lifecycle workspace create --project <id> --ref <branch> [--local]`
   - default: `--local` from CLI (cloud mode deferred to M7)
...
No new desktop app surfaces in M6 -- the CLI is the primary interface for this milestone.
