...
Key M1 requirements:
- JSONC-aware parser (comments and trailing commas permitted)
- JSON Schema validation with field-level errors
- Parse and display: `setup` steps, `services` definitions, `secrets` declarations

### Local Storage

- Tauri SQLite (`rusqlite`) for project persistence
...
- **Config panel**: parsed services, setup steps, secrets overview
...
- Config panel shows parsed services, setup steps, secrets
