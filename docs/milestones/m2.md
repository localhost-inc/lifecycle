...
User creates a local workspace, then clicks "Run" and watches it progress through `creating → sleeping → starting → ready` with per-service health indicators.
...
- **Workspace status display**: creating → sleeping → starting → ready progression
