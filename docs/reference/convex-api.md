...
- `workspaceServices.update`: patches the `workspace_service` record. Accepts only writable fields: `exposure` (`internal|organization|local`), `portOverride` (nullable). Returns full service record including computed fields (`status`, `previewState`, `previewUrl`, `assignedPort`).
...
Desktop attach-helper semantics for cloud terminals are defined in [reference/cloud-terminal-attach.md](./cloud-terminal-attach.md).
