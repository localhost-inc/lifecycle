...
In repository development mode, the control plane defaults to the local API dev server instead of `https://control-plane.lifecycle.dev`. The standard root dev path (`just dev` or `bun dev`) launches the CLI-owned TUI against the normal local Lifecycle runtime without forcing repo-local bridge/control-plane overrides. The repo-local TUI loop (`just tui-local` or `bun run dev:tui:local`) loads `LIFECYCLE_DEV=1`, a stable per-repo `LIFECYCLE_RUNTIME_ROOT` under `~/.lifecycle/dev/`, `LIFECYCLE_API_URL=http://127.0.0.1:18787`, `LIFECYCLE_API_PORT=18787`, `LIFECYCLE_BRIDGE_URL=http://127.0.0.1:52300`, and `LIFECYCLE_BRIDGE_PORT=52300`, then reuses or starts the repo-local bridge/control-plane services as needed. Bridge and CLI clients resolve their control-plane base URL from that shared process environment. The per-repo runtime root isolates bridge runtime state such as registration and supervisor files away from the checkout; local worktree checkouts still use the canonical `~/.lifecycle/worktrees/<org_slug>/<repo_slug>/` layout, with `local` as the org slug when no active organization is selected.
...
OpenAPI contract publication follows the same boundary:

1. The bridge owns the canonical OpenAPI document generated from its routed route tree.
...
The project contract. Declares workspace prepare steps and can optionally declare a managed stack DAG: named `process`, `image`, and `task` nodes with dependencies, health checks, and ports.
