...
4. `cargo clippy --all-targets -W clippy::all` still reports follow-up cleanup, but the structural warning count dropped meaningfully after the refactor.
...
2. Continue decomposing large backend modules, especially `platform/git/status.rs` and `workspaces/harness.rs`, using the terminal module split as the reference pattern.
