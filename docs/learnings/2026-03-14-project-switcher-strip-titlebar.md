...
3. the project layout then inherited a broken top-left boundary

## Decision

The shell plane now hosts a horizontal **project switcher strip** to the right of the window controls.

The project layout starts below that strip and renders as an inset raised card on top of the shell plane.
...
2. The project layout gets a clean rectangular boundary again.
3. Project switching stays shell-scoped without competing with the project sidebar.
4. The shell plane and the project layout now read as two intentional layers instead of a clipped single surface.

## Milestone Impact

1. Phase 1 of `execution/project-shell-cutover.md` now uses a top shell switcher instead of a left rail.
2. This sharpens the project-shell model without changing workspace ownership or the Phase 4 canvas cutover.

## Follow-Up

1. Keep project and organization switching in the shell strip only; do not move project page navigation into that area.
2. Preserve the inset-card treatment for the project layout so the shell plane remains legible as its own layer.
