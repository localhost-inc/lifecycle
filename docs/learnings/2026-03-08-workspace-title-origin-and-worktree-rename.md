...
2. Workspace rename cannot be treated as a display-only mutation.
...
3. Existing terminal sessions need their launch worktree path preserved.
...
2. M5 lifecycle work should treat worktree rename as ordinary local metadata maintenance, not a special destructive flow.

## Follow-Up Actions

1. Add a focused integration test around renaming a workspace while an attached harness session is still active.
2. Revisit whether the selected workspace title in the top bar should also enter inline edit mode, or remain a read-only mirror of the sidebar rename control.
