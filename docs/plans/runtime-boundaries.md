...
3. CLI commands must not depend on removed custom-agent/provider runtime paths.
...
2. Host adapters, stack runtime, terminal runtime, auth passthrough, and workspace orchestration live here.
...
3. Packaged desktop mode launches the bundled CLI helper by absolute path instead of re-implementing bridge orchestration in Swift.
...
2. The CLI no longer exposes first-party custom-agent commands.
3. Bridge routes and worker flows for the removed custom-agent runtime are out of the active packaging path.
4. `packages/agents` is no longer part of the active runtime boundary.
...
4. Keep custom-agent/provider runtime work out of the active CLI and desktop contracts.

## Remaining Cleanup

1. Remove or archive remaining legacy docs that still describe the deleted custom-agent runtime as active product scope.
2. Keep trimming desktop presentation code that only existed for the removed custom-agent surface.
...
3. no active client depends on a separate bridge package or first-party custom-agent runtime
