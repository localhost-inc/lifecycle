...
describe("lifecycle cli", () => {
  test("prints dynamic root help with no args", async () => {
    ...
    expect(sink.stdout[0]).toContain("bridge");
  ...
  test("destroys a named workspace through bridge-owned archive policy", async () => {
    const sink = createIo();
    await withHttpBridge(
      async (request) => {
        expect(request).toMatchObject({
  ...
  test("reports bridge-backed install status as json", async () => {
    const sink = createIo();
