...
final class WorkspaceChromeTests: XCTestCase {
  ...
  @MainActor
  func testAppSidebarRepositoryExpansionIsTrackedInModel() {
    let model = AppModel()

    model.setAppSidebarRepositoryExpanded(true, repositoryID: "repo-1")
    model.setAppSidebarRepositoryExpanded(false, repositoryID: "repo-2")

    XCTAssertTrue(model.isAppSidebarRepositoryExpanded("repo-1"))
    XCTAssertFalse(model.isAppSidebarRepositoryExpanded("repo-2"))

    model.toggleAppSidebarRepositoryExpanded("repo-1")

    XCTAssertFalse(model.isAppSidebarRepositoryExpanded("repo-1"))
  }

  func testAppSidebarLayoutStoreRoundTripsState() throws {
    let rootURL = FileManager.default.temporaryDirectory
      .appendingPathComponent("lifecycle-app-sidebar-\(UUID().uuidString)")
    defer {
    ...
    let restored = try AppSidebarLayoutStore.read(
      environment: ["LIFECYCLE_ROOT": rootURL.path, "HOME": NSHomeDirectory()]
    )
