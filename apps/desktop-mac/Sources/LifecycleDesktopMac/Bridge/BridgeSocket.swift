...
final class BridgeSocket {
  ...
  enum Event {
    ...
    case activity([BridgeWorkspaceActivity])
  ...
  /// Subscribe to additional bridge topics beyond the default `activity`.
  ...
  private func handleMessage(_ message: URLSessionWebSocketTask.Message) {
    ...
    let event = decodeEvent(type: type, object: object)
  ...
  private func decodeEvent(type: String, object: [String: Any]) -> Event {
    switch type {
    case "connected":
    ...
    case "activity":
      if let workspacesRaw = object["workspaces"],
         let data = try? JSONSerialization.data(withJSONObject: workspacesRaw),
         let workspaces = try? JSONDecoder().decode([BridgeWorkspaceActivity].self, from: data)
      {
        return .activity(workspaces)
      }
