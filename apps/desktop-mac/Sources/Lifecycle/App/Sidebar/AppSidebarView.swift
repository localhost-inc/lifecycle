        ...
        VStack(alignment: .leading, spacing: 2) {
          ForEach(model.repositories) { repository in
            AppSidebarRepositorySection(
              ...
              isExpanded: model.isAppSidebarRepositoryExpanded(repository.id),
              onToggleExpanded: { model.toggleAppSidebarRepositoryExpanded(repository.id) }
            )
          }
        }
      ...
      if let errorMessage = model.errorMessage {
        ...
        .background(
          RoundedRectangle(cornerRadius: 8, style: .continuous)
            .fill(theme.surfaceBackground.opacity(0.72))
        )
        .overlay {
          RoundedRectangle(cornerRadius: 8, style: .continuous)
