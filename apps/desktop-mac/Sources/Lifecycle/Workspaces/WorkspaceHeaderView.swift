...
private struct WorkspaceHeaderActionRow: View {
  ...
  var body: some View {
    HStack(spacing: 6) {
      WorkspaceHeaderLayoutTrigger(isActive: isViewSettingsPresented) {
...
private struct WorkspaceHeaderLayoutTrigger: View {
  @Environment(\.appTheme) private var theme
  let isActive: Bool
  let action: () -> Void
  ...
  var body: some View {
    Button(action: action) {
      Image(systemName: "square.grid.2x2")
        ...
        .frame(width: 22, height: 30, alignment: .trailing)
        .contentShape(Rectangle())
    }
    .buttonStyle(.plain)
    .lcPointerCursor()
