    ...
    GeometryReader { geometry in
      ...
      .overlay(alignment: .topLeading) {
        CanvasSpatialCreateBar(
          ...
          createTerminal: { kind in
            model.createTerminalTab(kind: kind, workspaceID: workspaceID)
          }
        )
        .padding(16)
      }
...
private struct CanvasSpatialCreateBar: View {
  ...
  let createTerminal: (BridgeTerminalKind) -> Void
        ...
        Menu {
          ForEach(surfaces) { surface in
            Button {
              ...
              Label(surface.tabPresentation.label, systemImage: surface.tabPresentation.icon)
            }
          }
        } label: {
          HStack(spacing: 8) {
            Image(systemName: activeSurface.tabPresentation.icon)
