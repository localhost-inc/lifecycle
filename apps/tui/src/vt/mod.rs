...
pub trait VtBackend {
    ...
    /// Return terminal-generated bytes that need to be written back to the PTY.
    fn take_pending_pty_writes(&mut self) -> Vec<u8> {
        Vec::new()
    }

    /// Whether this backend responds to terminal queries natively.
    fn uses_native_query_responses(&self) -> bool {
        false
    }

    /// Encode a keyboard event using the backend's current terminal mode state.
    fn encode_key(&mut self, _key: KeyEvent) -> Vec<u8> {
        Vec::new()
    }
