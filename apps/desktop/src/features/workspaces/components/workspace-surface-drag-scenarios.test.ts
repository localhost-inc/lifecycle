...
import { inspectWorkspacePaneLayout } from "../lib/workspace-surface-panes";
import {
  createFileViewerTab,
...
import {
  ...
  type WorkspaceSurfaceResolvedPaneDropState,
...
): WorkspaceSurfaceState["documentsByKey"] {
...
): Pick<WorkspaceSurfaceState, "paneTabStateById" | "rootPane"> {
...
function createPaneGeometry({
  ...
  top: number;
  width: number;
}): WorkspaceSurfacePaneDropGeometry {
  return {
    bodyRect: {
      bottom: top + height,
...
function applyWorkspaceTabDragStep(
...
): { nextState: WorkspaceSurfaceState; resolved: WorkspaceSurfaceResolvedPaneDropState } {
  const resolved = resolveWorkspaceSurfacePaneDropStateFromGeometry({
    draggedKey,
    paneGeometries,
    paneId: sourcePaneId,
    pointerX,
  ...
  if (resolved.intent.kind === "split") {
    ...
    const splitState = workspaceSurfaceReducer(state, {
    ...
    return {
      nextState: workspaceSurfaceReducer(splitState, {
  ...
  return {
    nextState: workspaceSurfaceReducer(state, {
...
describe("workspace surface drag scenarios", () => {
  ...
  test("nested split drag, resize, and close sequences keep pane topology coherent", () => {
    ...
    state = workspaceSurfaceReducer(state, {
