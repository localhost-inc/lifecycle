...
export interface AgentPromptInputQueuedPrompt {
...
export interface AgentPromptInputProps {
  ...
  queuedPrompts: AgentPromptInputQueuedPrompt[];
  planMode: boolean;
  onAddImagesFromFiles: (files: FileList | File[]) => void;
  onDismissQueuedPrompt: (id: string) => void;
  onRemovePendingImage: (index: number) => void;
  onDraftPromptChange: (value: string) => void;
  onRetryQueuedPrompt: (id: string) => void;
  onSend: () => void;
}
  ...
  function AgentPromptInput(
    ...
    const handleSelectFile = useCallback(
      (filePath: string) => {
        ...
        if (trigger.trigger === "@") {
          ...
          const newValue =
            draftPrompt.slice(0, trigger.startIndex) +
            insertion +
            draftPrompt.slice(trigger.endIndex);
    ...
    return (
      <div
        className={
          isCentered
            ? "w-full cursor-text bg-[var(--surface-hover)]/50"
            : "shrink-0 cursor-text bg-[var(--surface-hover)]/50"
        }
        onClick={() => textareaRef.current?.focus()}
      >
        {queuedPrompts.length > 0 ? (
          <div className="border-b border-[var(--border)]/60 px-4 pt-2 pb-1">
            {queuedPrompts.map((prompt) => (
              <div key={prompt.id} className="flex items-start gap-2 py-1.5">
                <span className="shrink-0 pt-[3px] text-[11px] text-[var(--muted-foreground)]/55">
                  &#9654;
                </span>
                <div className="min-w-0 flex-1">
                  <pre className="whitespace-pre-wrap break-words font-[var(--font-mono)] text-[12px] leading-5 text-[var(--muted-foreground)]/80">
                    {prompt.text}
                  </pre>
                  {prompt.attachmentSummary ? (
                    <div className="pt-0.5 text-[11px] text-[var(--muted-foreground)]/65">
                      {prompt.attachmentSummary}
                    </div>
                  ) : null}
                  {prompt.error ? (
                    <div className="flex items-center gap-3 pt-1 text-[11px]">
                      <span className="text-[var(--destructive)]">{prompt.error}</span>
                      <button
                        className="text-[var(--muted-foreground)] transition-colors hover:text-[var(--foreground)]"
                        onClick={() => onRetryQueuedPrompt(prompt.id)}
                        type="button"
                      >
                        Retry
                      </button>
                      <button
                        className="text-[var(--muted-foreground)] transition-colors hover:text-[var(--foreground)]"
                        onClick={() => onDismissQueuedPrompt(prompt.id)}
                        type="button"
                      >
                        Dismiss
                      </button>
        ...
        <div
          className={
            isCentered
              ? "flex cursor-text items-start px-5 pt-4 pb-3"
              : "flex cursor-text items-start px-4 pt-3 pb-2"
          }
          onClick={() => textareaRef.current?.focus()}
        >
          ...
          <div className="relative min-w-0 flex-1">
            ...
            <textarea
              ...
              rows={isCentered ? 2 : 1}
              style={isCentered ? { height: "auto", minHeight: "3.5rem" } : { height: "auto" }}
              value={draftPrompt}
            />
            {showCursorBlink ? (
